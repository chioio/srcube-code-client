import { Editor } from "@components/base"

interface CodePanelProps {
  lang: 'html' | 'css' | 'javascript'
}

export const CodePanel: React.VFC<CodePanelProps> = ({ lang }) => {
  let icon: JSX.Element = <></>
  let hint = ''
  switch (lang) {
    case 'html': {
      icon = <HtmlSVG w={20} h={20} />
      hint = '<!-- Write your html code here... -->'
      break
    }
    case 'css': {
      icon = <CssSVG w={20} h={20} />
      hint = '/* Write your css code here... */'
      break
    }
    case 'javascript': {
      icon = <JavascriptSVG w={20} h={20} />
      hint = '// Write your script code here...'
      break
    }
  }
    ...
    <div className="flex flex-col overflow-hidden rounded-xl">
      <div className="flex-none flex items-center py-2 px-3 w-full border-b border-gray-100 bg-white dark:bg-gray-900">
        <div className="flex items-center">
          {icon}
          <h2 className="ml-2 font-bold text-gray-700 dark:text-gray-200">{lang.toUpperCase()}</h2>
        </div>
      </div>
      <div className="h-0 flex-grow">
        <Editor lang={lang} hint={hint} />
      </div>
    </div>
  )
}
...
const JavascriptSVG = ({ w, h }: { w: number; h: number }) => (
  <svg
    aria-hidden="true"
    focusable="false"
    data-icon="css3"
